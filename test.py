...
if __name__ == "__main__":
    df = pd.read_csv('D:\hk1-gd1-4\mechineleaning\indian_liver_patient_dataset.csv', index_col = 0, parse_dates = True)
    X = df.iloc[:, :-1]
    y = df.iloc[:, -1]

    xtrain, xtest, ytrain, ytest = train_test_split(X,y,train_size=0.7, shuffle = False)    
    tree = DecisionTreeID3(max_depth = 4, min_samples_split = 2)
